...
"""
    ...
    for lmbda in lmbdas:
        ...
        res["MSE_validation"]=aux["mse_test"] # Note that you should use mse_test!!
        lmb_vs_MSE.append(res)

    df=pd.DataFrame(lmb_vs_MSE)
    
    best_validation_mse= df["MSE_validation"].min()
    best_lmbda= df["lambda"][df["MSE_validation"]==best_validation_mse].item()
...
if __name__ == "__main__":
    ...
    alphas =  np.arange(2, 6.01, 0.01).tolist()
    ...
    alphas = np.arange(0.01, 2.51, 0.01).tolist()
    lmbda = 0.001
    plot_dataset_size_vs_mse(X_train, y_train, X_test, y_test,alphas,lmbda,regularization, filename='dataset_size_vs_mse_l2=001')

    lmbda = 10.0
    plot_dataset_size_vs_mse(X_train, y_train, X_test, y_test,alphas,lmbda, regularization, filename='dataset_size_vs_mse_l2=10')

    ## Exercise 5.
    print('Plotting regularizer vs. coefficient curve...')
    lmbdas = np.arange(0, 5000.01, 0.1).tolist()
    plot_coeffs = [0,3,7,8]
    ...
    lmbdas = np.arange(0,100.1,0.1).tolist()
    n_ = 80
    ...
    lmbdas = np.linspace(0.0001, 1.2,100)
    ...
    plot_coeffs = [0,2,6,7,29+size,54+size]
    plot_regularizer_vs_coefficients(X_train_poly, y_train, X_test_poly, y_test, lmbdas,  plot_coeffs,
     regularization='lasso', filename='regularizer_vs_coefficients_LASSO_polyfeat')
